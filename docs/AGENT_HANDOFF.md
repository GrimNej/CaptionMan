...
## Current State
- Current milestone: Final public submission image is pushed and verified, the final 10-slide presentation PDF is built and QA-checked, a separate showcase video kit has been prepared, and the Oracle VPS demo is publicly live at `https://captionman.grimnej.com/studio`. Judged Docker behavior remains independent from the frontend.
- Current branch: `main`
- Last completed task: Verified public DNS, ingress, TLS, API health, doctor, and browser loading for the VPS demo at `https://captionman.grimnej.com/studio`.
- Last passing commands: Public checks: DNS resolve for `captionman.grimnej.com`, `Test-NetConnection` for TCP 80/443, `Invoke-WebRequest https://captionman.grimnej.com/api/health`, `Invoke-WebRequest https://captionman.grimnej.com/api/doctor?live=false`, `Invoke-WebRequest https://captionman.grimnej.com/studio`, and a Playwright browser smoke test that loaded the live Studio. Earlier VPS checks: server `docker compose up -d`, internal health/studio/doctor checks, and v2 kitten URL canary through the deployed API. Local checks: `uv run pytest tests/test_demo_api_artifacts.py tests/test_run_input_resolution.py` from `apps/api`.
