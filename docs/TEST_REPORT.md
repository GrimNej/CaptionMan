...
### 2026-07-11 11:55
- Command: Participant-guide scoring audit; baseline `uv run captionman run-one E:\Trmp\348656_medium.mp4`; corrected holdout and v1-v3 direct-provider runs; `uv run captionman doctor`; `uv run ruff check .`; `uv run ruff format --check .`; `uv run pytest`; official result validation; artifact scans; source-hygiene and no-secrets scans.
- Environment: Windows PowerShell, uv-managed CPython 3.13.12, Fireworks direct route with Kimi K2.6 vision and GLM 5.2 caption writing, ignored local temporary hackathon key.
- Result: pass
...
### 2026-07-11 13:05
- Command: Final embedded-key image build; exact-image doctor; mounted mock run; full real-provider v1-v3 run; result validation and quality scans; Docker Hub push; anonymous manifest inspection and pull; published-image doctor; published-image mounted mock run and validation; `uv run pytest`; Ruff lint/format checks; no-secrets and source-hygiene scans.
- Environment: Docker Desktop Linux engine, `linux/amd64`, Fireworks Kimi/GLM champion route, approved embedded temporary hackathon credential, and an empty Docker credential directory for registry checks.
- Result: pass
- Output summary: The exact final image completed v1-v3 in 95.06 seconds with all three tasks and all 12 requested captions. Input-aware schema validation, ASCII checks, minimum-content checks, and scans for model chatter, frame artifacts, generic fallbacks, speculative intent, production-style hallucinations, uncertain workstation labels, incidental hairstyle, and vague color qualifiers all passed. Docker Hub anonymously served and pulled digest `sha256:c5faf2947724f36e186ba5b03eaa700de91d5fb4328816d6561cacb521966af1` as a plain Docker v2 `linux/amd64` manifest. Registry size is 277.21 MiB. The pulled image reports the correct default command, passed live provider doctor, wrote official mock results through its default command, and passed `scripts/validate_results.py`. The backend finished with 62 passing tests; Ruff, no-secrets, and source-hygiene checks passed.
- Failures: The first metadata-print command used an invalid Docker Go-template expression after a successful anonymous pull; a manifest parser initially assumed a multi-platform index even though the intentionally single-platform manifest is valid.
- Fixes attempted: Parsed verbose single-platform manifest metadata directly and reran image inspection with PowerShell object formatting. No image rebuild or runtime change was needed.

### 2026-07-11 18:35
- Command: Qwen/Kimi route calibration; focused and full backend tests; Ruff checks; repository secret/hygiene scans; guarded final Docker build; exact-image doctor, mock, and real v1-v3 runs; result validation and manual caption review; Docker Hub push; anonymous manifest inspection and pull; published-image doctor and mock validation.
- Environment: Windows PowerShell, uv-managed backend, Docker Desktop Linux engine, `linux/amd64`, Qwen3.7 Plus primary vision, Kimi K2.7 evidence fallback, GLM 5.2 caption writing, approved temporary embedded Fireworks credential, and a credential-free Docker config for registry verification.
- Result: pass
- Output summary: Backend completed 79 tests; Ruff lint/format, no-secrets, and source-hygiene checks passed. The exact final image produced 3 task results and all 12 requested captions in 56.43 seconds, with 12-25 words per caption, correct sentence capitalization, explicit scene anchors, and schema-valid JSON. Anonymous Docker Hub access returned and pulled digest `sha256:0e672ddfcf898971344043be5d29bffa01de132e15e6e929cf84e876608fdfae`; the 277.20 MiB plain Docker v2 manifest is `linux/amd64`, has the expected default command, passes live provider doctor, and writes validator-clean mounted mock results.
- Failures: A pre-release v3 technical caption stated an unseen code deployment literally; an early Docker inspection command referenced an absent entrypoint key; a later Go-template expression for repository digests was invalid. Neither Docker metadata command failure affected the built image or runtime.
- Fixes attempted: Added evidence-anchor overlap and figurative-tech postconditions plus regression tests, rebuilt and repeated every release gate, and reran metadata inspection using PowerShell JSON objects. The unsafe technical claim was replaced by a clearly figurative evidence-preserving caption without another model call.

### 2026-07-11 18:55
- Command: Git-only VPS source archive and hash verification; remote Compose validation; API-only image rebuild/recreate; internal and public health checks; public doctor inspection; real hosted v2 URL runs; Judge Replay artifact and rendered page checks.
- Environment: Oracle Ubuntu VPS at the isolated `/opt/captionman` deployment, Docker Compose, Caddy HTTPS, server-only Fireworks env file, and the committed Qwen/Kimi/GLM source tree.
- Result: pass
- Output summary: Existing web and Caddy containers remained up during the API build. Internal and public health returned `ok`; public doctor reported the intended champion route and inactive Gemma. After Compose pinned the frame profile, run `run_1783775201_77e997b4` completed in 56.12 seconds with 10 chronological images, two model calls, a grounded ginger-kitten summary, four distinct captions, and a 200 Judge Verdict page containing the relevant kitten caption and no kitchen placeholder.
- Failures: The first hosted canary used six images because the private server env file retained an older `NUM_FRAMES` value. One diagnostic shell command also failed from local PowerShell parsing a remote grep alternation; health and service state were unaffected.
- Fixes attempted: Moved all scoring-critical frame and recovery values into Compose, recreated only the API container, verified effective values individually without printing credentials, and repeated the real canary plus replay inspection.

### 2026-07-12 09:30
- Command: Nine-page FAQ extraction/render review; retired v4-v8 download/probe/frame inspection; unchanged-image baseline; focused/full backend and web tests; Ruff/Biome/build; secret/hygiene scans; v3 and v4 replay canaries; key-free Docker boundary/mock checks; guarded embedded-key builds; exact public-digest v1-v8 run and validator; Docker Hub push; anonymous manifest/digest pull; VPS deploy; public frame checks; Playwright visual and console verification.
- Environment: Windows PowerShell, Poppler, ffmpeg/ffprobe, uv-managed backend, Docker Desktop Linux engine, Qwen3.7 Plus, Kimi K2.7 fallback, GLM 5.2, approved temporary embedded key, and credential-free Docker registry config.
- Result: pass
- Output summary: All FAQ pages rendered legibly and confirmed that public examples are retired while hidden prompts/cases remain unavailable. The exact public digest completed eight tasks and 32 captions in 199.8 seconds, preserved every task ID, returned four unique styles per task, used 15-25 words per caption, and passed input-aware schema and artifact scans. The backend reached 83 passing tests. Anonymous pull passed for plain Docker v2 digest `sha256:756a80fa9de66565476b4b50d4b5624e21dc3857483990329f55718e9105fe11`; compressed size is 277.20 MiB. Hosted HTML used public frame URLs, all 10 frame endpoints returned JPEGs, and Playwright reported zero browser errors or warnings.
- Failures: The initial v3 confidence canary repeated an incidental high-bun detail and one fallback omitted the article before `woman`. An earlier eight-task run showed that one recovery can leave later unsafe styles on generic fallbacks. Browser verification found server-rendered internal frame URLs and a missing favicon. The FAQ's final style checklist also contains a label inconsistent with its schema/examples.
- Fixes attempted: Preserved only confidently supported broad person categories, expanded incidental hair normalization without leaving fragments, naturalized bare person fallbacks, routed semantic failures into recovery, raised recovery allowance to three under the unchanged eight-call cap, retained the schema-correct `humorous_non_tech` key, separated server fetch and browser asset origins, and added icon metadata.
