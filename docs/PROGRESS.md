...
### 2026-07-11 18:35
- Milestone: Qwen champion release with semantic postconditions
- Summary: Completed public-example calibration without hardcoded answers or competitor inspection, promoted Qwen3.7 Plus with one budget-counted Kimi K2.7 evidence fallback, preserved compact scene anchors, and added deterministic evidence-overlap and figurative-tech gates after direct output review found one literal unseen software claim.
- Files changed: Provider routing and normalization, caption safety and batch prompt, Docker/deployment defaults, tests, README routing diagram, and living docs.
- Commands run: Official-guide and official-event scoring audit; Fireworks serverless model checks; bounded Qwen/Kimi/MiniMax calibration runs; 79-test backend suite; Ruff lint/format; no-secrets and source-hygiene scans; guarded embedded-key `linux/amd64` build; exact-image doctor; mounted mock run plus validator; exact-image real v1-v3 run; Docker Hub push; credential-free manifest inspection and pull; published-digest doctor and mounted mock validation.
- Result: The final exact image produced all 12 required captions in 56.43 seconds and passed schema/output review. Docker Hub anonymously serves digest `sha256:0e672ddfcf898971344043be5d29bffa01de132e15e6e929cf84e876608fdfae` as a plain Docker v2 `linux/amd64` manifest. The pulled image reports the correct default command, passes live Fireworks doctor with Qwen3.7 primary, Kimi K2.7 fallback, GLM writing, and the embedded temporary credential, and writes validator-clean output in mock mode.
- Next step: Re-save or resubmit `docker.io/grimnej/captionman:submission` so the evaluator pulls the replacement digest. Treat the next public score as authoritative; no top-three result is claimed in advance.

### 2026-07-11 18:55
- Milestone: Hosted Studio aligned with judged quality route
- Summary: Synced committed source to the isolated `/opt/captionman` VPS deployment, rebuilt only the API service, and pinned scoring-critical route/frame/recovery settings in Compose after replay exposed a stale six-frame server env override.
- Files changed: `deploy/vps/docker-compose.yml` and living docs.
- Commands run: Git-only deployment archive and SHA-256 verification; remote Compose validation; API-only rebuild/recreate; internal and public health checks; public lightweight doctor; two real hosted URL canaries; replay frame/call inspection; public Judge Verdict HTML checks.
- Result: Caddy and web stayed online while the API rebuilt. Public doctor reports Qwen3.7 Plus, Kimi K2.7 fallback, GLM 5.2, `linux/amd64`, and Gemma inactive. The post-lock v2 canary completed in 56.12 seconds using 10 chronological images and two calls; captions were grounded and style-distinct, and its Judge Verdict page returned 200 with kitten content and no stale kitchen placeholder.
- Next step: Re-save or resubmit the unchanged Docker tag. The hosted demo is ready for manual review at `https://captionman.grimnej.com/studio`.

### 2026-07-12 09:30
- Milestone: Judging FAQ and eight-clip public-validation release
- Summary: Extracted and visually reviewed all nine FAQ pages, mapped every Track 2 and failure-code criterion, downloaded and inspected the five newly listed retired clips, ran the unchanged image for a baseline, and made only hidden-set-general reliability changes. No public reference caption or sample mapping was added.
- Files changed: Pipeline recovery, evidence normalization, caption safety, evidence/style prompts, Docker/VPS/launcher recovery defaults, focused tests, `docs/JUDGING_FAQ_AUDIT.md`, README, submission docs, and living docs.
- Commands run: PDF metadata/text extraction; 9-page Poppler render and visual review; public video HEAD/download/probe checks; v4-v8 baseline Docker run; source-frame contact-sheet review; focused and full backend suites; web tests/lint/build; Ruff; no-secrets and source-hygiene scans; v3/v4 replay canaries; key-free Docker boundary/mock gates; guarded final builds; exact-image doctor/mock; exact public-digest v1-v8 run; input-aware validation; caption artifact scans; Docker Hub push; anonymous manifest inspection and digest pull; VPS API/web deploy; public frame checks; Playwright visual/console checks.
- Result: The final public digest produced all eight tasks and 32 distinct requested captions in 199.8 seconds. Captions were 15-25 words, specific to visible content, style-distinct, schema-valid, and clean of model chatter, frame language, generic video fallbacks, and incidental hairstyle leakage. Docker Hub anonymously serves plain Docker v2 `linux/amd64` digest `sha256:756a80fa9de66565476b4b50d4b5624e21dc3857483990329f55718e9105fe11` at 277.20 MiB. Backend finished with 83 passing tests; the hosted replay serves all 10 office frames and Playwright reports zero errors or warnings.
- Next step: Re-save or resubmit `docker.io/grimnej/captionman:submission` once so the evaluator pulls this digest. Do not repeatedly resubmit while the leaderboard queue catches up.
